...
The [`jelly sample/`](jelly%20sample/) Xcode project in this repo is the canonical example. Files touched in a host app for a normal integration:

- `<your-app>.xcodeproj/project.pbxproj` (or `Package.swift`) — local Swift Package reference + library product on the app target
...
1. **Source-level gate.** Wrap every `import Jelly` and `Jelly.install()` call in `#if DEBUG`. Compiles in any project setup. Used by the included sample app.
